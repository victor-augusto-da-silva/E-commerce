...
def add_to_cart(product_id):
    ...
    if user and product:
        cart_intem = CartItem(user_id=user.id, product_id= product.id)
        db.session.add(cart_intem)
        db.session.commit()
        return jsonify({"message":"Intem added to the cart successfully"})
    return jsonify({"messa":"Failed to add item to the cart"}),400
